...
import { ArrowLeft, Plus, X, Users, User, Shuffle, Play, CheckSquare, Square } from 'lucide-vue-next'
...
const activePlayers = computed((): IPickerPlayer[] => players.value.filter(p => p.isActive))
...
const unassignedPlayers = computed((): IPickerPlayer[] => players.value.filter(p => p.teamId === null))
...
function addPlayer(): void {
   ...
   if (name === '') return
   players.value.push({ id: generateId(), name, isActive: true, teamId: null })
   newPlayerName.value = ''
}
...
function toggleAllPlayers(active: boolean): void {
   players.value.forEach(p => { p.isActive = active })
}

function addTeam(): void {
   const name = newTeamName.value.trim()
   if (name === '') return
   teams.value.push({ id: generateId(), name, color: getColorByIndex(teams.value.length) })
   newTeamName.value = ''
}

function removeTeam(id: string): void {
   players.value.forEach(p => { if (p.teamId === id) p.teamId = null })
         ...
         >
            ...
            <div class="flex gap-2">
               <UiInput
                  ...
                  @keyup.enter="addTeam"
               />
               <UiButton size="sm" @click="addTeam">
                  <Plus class="h-4 w-4" />
               </UiButton>
            </div>
            ...
            <div v-if="teams.length > 0" class="space-y-2">
               ...
               >
                  <div class="flex items-center gap-2 mb-2">
                     ...
                     <UiIconButton variant="destructive" size="sm" @click="removeTeam(team.id)">
                        <X class="h-4 w-4" />
                     </UiIconButton>
                  </div>
                  <div v-if="getTeamPlayers(team.id).length > 0" class="flex flex-wrap gap-1">
                     <UiBadge
                        ...
                        :class="{ 'opacity-50 line-through': !player.isActive }"
                     >
                        {{ player.name }}
                     </UiBadge>
                  </div>
            ...
            >
               <div class="flex items-center gap-3">
                  ...
                  <div class="player-list__actions flex gap-1">
                     ...
                     <UiIconButton variant="destructive" size="sm" @click="removePlayer(player.id)">
                        <X class="h-4 w-4" />
                     </UiIconButton>
                  </div>
               </div>
